...
if (showUnitTests) {
    ...
    descriptiveCounterTests_1.testDescriptiveCounter();
}
...
function UnitTestRemoveAsserts() {
    ...
    let input = ["ASSERT a;"];
    let expected = [""];
    UnitTest3(VHDLFormatter_4.RemoveAsserts, "one assert", input, expected);
    input = ["ASSERT a", "b;", "c"];
    expected = ["", "", "c"];
    UnitTest3(VHDLFormatter_4.RemoveAsserts, "multiline assert", input, expected);
}
...
function IntegrationTest49() {
    let settings = GetDefaultSettings();
    settings.SignAlignSettings = new VHDLFormatter_1.signAlignSettings(true, false, "", ["PROCEDURE"]);
...
function IntegrationTest50() {
    let settings = GetDefaultSettings();
    settings.SignAlignSettings = new VHDLFormatter_1.signAlignSettings(true, false, "", []);
    let input = 'PROCEDURE wait_until(\r\n    SIGNAL a : IN data_status;\r\n    b : data_status\r\n);';
    let actual = VHDLFormatter_1.beautify(input, settings);
...
function IntegrationTest51() {
    let settings = GetDefaultSettings();
    settings.SignAlignSettings = new VHDLFormatter_1.signAlignSettings(false, true, "", []);
...
function IntegrationTest79() {
    let settings = new VHDLFormatter_3.BeautifierSettings(false, false, false, null, "lowercase", "uppercase", null, null, "\r\n");
...
function IntegrationTest82() {
    let settings = GetDefaultSettings();
    settings.SignAlignSettings = new VHDLFormatter_1.signAlignSettings(false, true, "local", []);
    let input = 'long : test;\r\n-- comment\r\nloooooong : test;';
    let actual = VHDLFormatter_1.beautify(input, settings);
    assertAndCountTest("block alignment local", input, actual);
}
function IntegrationTest83() {
    let settings = GetDefaultSettings();
...
function GetDefaultSettings(indentation = "    ") {
    let new_line_after_symbols = new VHDLFormatter_2.NewLineSettings();
    new_line_after_symbols.newLineAfter = ["then", ";"];
    new_line_after_symbols.noNewLineAfter = ["generic"];
    let signAligns = new VHDLFormatter_1.signAlignSettings(false, false, "", []);
...
function getDefaultBeautifierSettings(newLineSettings, signAlignSettings = null, indentation = "    ") {
    return new VHDLFormatter_3.BeautifierSettings(false, false, false, signAlignSettings, "uppercase", "uppercase", indentation, newLineSettings, "\r\n");
}
...
function IntegrationTest5() {
    let settings = GetDefaultSettings();
    settings.SignAlignSettings = new VHDLFormatter_1.signAlignSettings(true, false, "", ["PORT"]);
    let input = "port map(\r\ninput_1 => input_1_sig,\r\ninput_2 => input_2_sig,\r\noutput => output_sig\r\n);";
    let expected = "PORT MAP(\r\n    input_1 => input_1_sig,\r\n    input_2 => input_2_sig,\r\n    output  => output_sig\r\n);";
    let actual = VHDLFormatter_1.beautify(input, settings);
